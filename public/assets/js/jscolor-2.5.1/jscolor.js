...
var jsc = {
	...
	pub : function (targetElement, opts) {
		...
		this.randomize = function (minV, maxV, minS, maxS, minH, maxH, minA, maxA) {
			...
			this.fromHSVA(
				minH + Math.floor(crypto.getRandomValues(array) * (maxH - minH + 1)),
				minS + Math.floor(crypto.getRandomValues(array) * (maxS - minS + 1)),
				minV + Math.floor(crypto.getRandomValues(array) * (maxV - minV + 1)),
				((100 * minA) + Math.floor(crypto.getRandomValues(array) * (100 * (maxA - minA) + 1))) / 100
			);
		};
		...
		this.toString = function (format) {
			...
			switch (format.toLowerCase()) {
				case 'hex': functionName = this.toHEXString(); break;
				case 'hexa': functionName = this.toHEXAString(); break;
				case 'rgb': functionName = this.toRGBString(); break;
				case 'rgba': functionName = this.toRGBAString(); break;
